...
def movement(player_pos, game_board_pos, game_board_col, discovered_pos, undiscovered_pos):
    ...
    trap_count = 3
    alive = True
    traps = {"trap_1": "inventory", "trap_2": "inventory", "trap_3": "inventory"}
    trap_timer = {"trap_1": "inventory", "trap_2": "inventory", "trap_3": "inventory"}
    while alive:
        ...
        if move == True:
            trap(trap_count, player_pos, traps, trap_timer)
...
def compass(direction, new, discovered_pos):
    ...
    food = input("Trap set off?: (Enter for no) ").lower() or 'none'


    # Changes the compass brackets and sets the direction moved
    if direction == 'w':
    ...
    # If a trap is set off, the message is displayed
    # Will be found off position list in final program
    
    if food == 'none':
        food_avaliable = ""
    else:
        food_avaliable = "One of your traps snap shut..."
                                              ...
                                              `.     {}     .`         
                                                `.           .`     
                                                  `.       .`            
                                                    `.   .`             
                                                      `.`                                                    
    """.format(direction_moved, space.ljust(5-len(direction_moved)), news[0], food_avaliable, space.ljust(37-(len(food_avaliable))), news[1], news[2], description, space.ljust(33-len(description)), discovered_num, space.ljust(2-len(str(discovered_num))), news[3]))


def trap(trap_count, player_pos, traps, trap_timer):
    repeat = True

    if trap_count > 0:
        print("You have {} traps remaining.".format(trap_count))
        while repeat:
            trap_down = input("Do you want to place a trap before you leave? Y/N (Enter to continue): ").lower()
            if trap_down in ('y', 'yes'):
                repeat = False
                print("You set a trap down at your feet...")
            elif trap_down in ('n', 'no', ''):
                repeat = False
            else:
                print("That input was invalid")
                
        if trap_down in ('y', 'yes'):
        # Finds the first trap avaliable and sets it down on the player's position
        # Sets a random timer for the trap
            if traps["trap_1"] == "inventory":
                traps["trap_1"] = player_pos
                trap_timer["trap_1"] = random.randint(1,4)
                
            elif traps["trap_2"] == "inventory":
                traps["trap_2"] = player_pos
                trap_timer["trap_2"] = random.randint(1,4)
                
            else:
                traps["trap_3"] = player_pos
                trap_timer["trap_2"] = random.randint(1,4)
        ...
        print("You have 0 traps remaining.")

    if traps["trap_1"] != "inventory":
        print("Trap 1 out")
    if traps["trap_2"] != "inventory":
        print("Trap 2 out")
    if traps["trap_3"] != "inventory":
        print("Trap 3 out")
